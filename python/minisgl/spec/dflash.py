...
class DFlashProposer(Proposer):
    ...
    def _load_draft_weights(self, folder: str) -> None:
        ...
        for i, layer in enumerate(d.layers):
            ...
            assign(layer.q_proj, "weight", p + "self_attn.q_proj.weight")
            assign(layer.k_proj, "weight", p + "self_attn.k_proj.weight")
            assign(layer.v_proj, "weight", p + "self_attn.v_proj.weight")
            assign(layer.o_proj, "weight", p + "self_attn.o_proj.weight")
            assign(layer.q_norm, "weight", p + "self_attn.q_norm.weight")
            assign(layer.k_norm, "weight", p + "self_attn.k_norm.weight")
            assign(layer.gate_proj, "weight", p + "mlp.gate_proj.weight")
            assign(layer.up_proj, "weight", p + "mlp.up_proj.weight")
            assign(layer.down_proj, "weight", p + "mlp.down_proj.weight")

        if self._compressed:
            assert "embed_tokens.weight" in sd, "compressed DFlash ckpt missing embed_tokens"
            d.set_own_embed(sd["embed_tokens.weight"].to(self._dtype).contiguous())
            assign(d._own_lm_head, "weight", "lm_head.weight")
