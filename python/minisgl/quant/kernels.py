...
def rxf_moe(
    ...
    *,
    ...
    ``topk_weights``/``topk_ids`` route (GLM/DeepSeek noaux_tc computed in the model — the
    renormalize + scaling are already folded in, so pass them through unchanged)."""
    import torch.nn.functional as F
    import moe_hip
    ...
    # Precomputed route (GLM/DeepSeek noaux_tc) or torch softmax+topk fallback (matches the former
    # _moe_C.topk_softmax; the lean image has no vllm).
