...
class MoELayer(BaseOP):
    def __init__(
        ...
        if quant is not None:
            # int4 W4A8 grouped experts. The unfused-silu kernel + checkpoint layout currently
            # support only GPTQ-int4 SwiGLU MoE (matches the proven w4a8_fp8_wmma MoE path).
            assert quant.is_gptq, "MoE W4A8 supports GPTQ-int4 checkpoints only"
            assert activation == "silu" and not apply_router_weight_on_input, (
                "MoE W4A8 path is silu-only without router-weight-on-input"
            )
            self.gate_up_proj = _GroupedGPTQExperts(
                num_experts, 2 * intermediate_size_per_partition, hidden_size, quant
            )
            self.down_proj = _GroupedGPTQExperts(
