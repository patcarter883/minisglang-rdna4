...
__global__ void __launch_bounds__(MAX_THREADS) moe_gemm_tiled_kernel(
    ...
    for (int g = 0; g < num_groups; ++g) {
        ...
        // ---- stage weights: contiguous fp8 byte copy into B_tile[n][k] (uint = 4 e4m3) ----
        const int njobs = BN * (BK / 4);
        for (int j = tid; j < njobs; j += total_threads) {
            const int n = j / (BK / 4), k4 = j % (BK / 4);
            const int an = block_n + n;
            unsigned int v = (an < N)
                ? *reinterpret_cast<const unsigned int*>(&wq_e[(long)an * K + k0 + k4 * 4]) : 0u;
            *reinterpret_cast<unsigned int*>(&B_tile[n * LDSBK + k4 * 4]) = v;
        }
        __syncthreads();
        ...
        // W8A8: no per-group weight-scale fold; accumulate raw acc and apply the
        // per-output-channel weight scale once in the epilogue.
        #pragma unroll
        for (int f = 0; f < NFRAG_W; ++f)
            #pragma unroll
            for (int ee = 0; ee < 8; ++ee) running[f][ee] += acc[f][ee];
        __syncthreads();
    }

    // ---- epilogue: per-row act scale + per-N weight scale, then store or atomic scatter ----
    #pragma unroll
    for (int ee = 0; ee < 8; ++ee) {
        ...
        if constexpr (SCATTER) {
            ...
            for (int f = 0; f < NFRAG_W; ++f) {
                const int abs_n = block_n + (nfrag0 + f) * WMMA_DIM + frag_col;
                if (abs_n < N)
                    out[(long)row_pad * N + abs_n] =
                        __float2half(running[f][ee] * asc * ws_e[abs_n]);
            }
        }
    }
}
    ...
    for (int gt = 0; gt < num_groups; gt += gtile) {
        ...
        for (int gi = gt; gi < gend; ++gi) {
            ...
            for (int j = tid; j < njobs; j += total_threads) {
                ...
                unsigned int v = (an < N)
                    ? *reinterpret_cast<const unsigned int*>(&wq_e[(long)an * K + k0 + k4 * 4]) : 0u;
                *reinterpret_cast<unsigned int*>(&Bg[n * LDSBK + k4 * 4]) = v;
            }
        }
        __syncthreads();
        ...
        for (int gi = gt; gi < gend; ++gi) {
            ...
            // W8A8: accumulate raw acc; per-N weight scale applied once in the epilogue.
            #pragma unroll
            for (int f = 0; f < NFRAG_W; ++f)
                #pragma unroll
                for (int ee = 0; ee < 8; ++ee) running[f][ee] += acc[f][ee];
