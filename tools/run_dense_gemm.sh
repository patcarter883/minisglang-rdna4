#!/usr/bin/env bash
# gpu-lease -n 1 --wait --name dense-gemm -- bash tools/run_dense_gemm.sh
set -uo pipefail
cd "$(dirname "$0")/.."
docker run --rm \
  ...
  --entrypoint bash minisgl-rdna4:lean -lc "python /engine/tools/dense_gemm_validate.py"
echo "[dense-gemm] done rc=$?"
