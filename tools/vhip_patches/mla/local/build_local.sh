#!/usr/bin/env bash
# Local (no-Nix) build of mla_hip_C for greening. Run from the package root (mla/), inside the ROCm
# torch image (torch 2.10 / ROCm 7.x / hipcc for gfx1201). Produces:
#   torch-ext/mla_hip/mla_hip_C*.so   — the compiled extension
#   torch-ext/mla_hip/_ops.py         — a shim providing `ops` + `add_op_namespace_prefix`, so the
#                                        SAME __init__.py used for the Hub build imports here
# The Hub build (kernel-builder) generates its own _ops.py; this shim is git-ignored.
set -euo pipefail

ARCH="${GPU_ARCHS:-gfx1201}"
PKG_DIR="torch-ext/mla_hip"

echo ">> building mla_hip_C for ${ARCH}"
GPU_ARCHS="${ARCH}" python local/setup.py build_ext --inplace

# CUDAExtension --inplace drops mla_hip_C*.so at the cwd; move it next to the python package.
SO="$(ls mla_hip_C*.so 2>/dev/null | head -1 || true)"
if [ -z "${SO}" ]; then echo "ERROR: no mla_hip_C*.so produced" >&2; exit 1; fi
mv -f "${SO}" "${PKG_DIR}/"
echo ">> placed ${PKG_DIR}/${SO}"

# Local _ops shim: the extension registers under torch.ops.mla_hip_C (TORCH_EXTENSION_NAME=mla_hip_C).
