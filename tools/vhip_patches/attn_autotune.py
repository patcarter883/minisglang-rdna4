...
# ---------------------------------------------------------------------------------------------
# vhip PATCH — persistent on-disk cache for the ALWAYS-ON startup attention autotune.
#
# Upstream profiles up to 12 configs at EVERY cudagraph capture size for EVERY attention group on
# every boot, and only ever DUMPS the result (VLLM_ATTN_AUTOTUNE_DUMP_DIR) — there is no load path,
# and _PROFILE_CACHE is in-process only. That is the dominant boot cost when the GDN path is native
# HIP (the GDN Triton compile is gone, so this is what's left), and it makes an A/B matrix painful.
#
# This adds load-then-profile against a directory keyed by a hash of the EXISTING `cache_key` tuple
# (shape/dtype/kv-dtype/query_len/capture-sizes/block/max_len/prefill_qlen/device) — so a cache hit
# is only ever reused for a byte-identical tuning problem, and any config change re-profiles.
# Knobs are unchanged on a hit, so tuned performance is preserved; only the wait disappears.
#
#   VLLM_ATTN_AUTOTUNE_CACHE_DIR=<dir>   enable the persistent cache (mount it from the host)
#   VLLM_ATTN_AUTOTUNE=0                 skip profiling entirely -> upstream's built-in defaults
#                                        (FAST but UNTUNED — do not mix with tuned rows in a matrix)
_ATTN_AUTOTUNE_OFF = os.environ.get("VLLM_ATTN_AUTOTUNE", "1") == "0"


def _disk_cache_path(cache_key):
    d = os.environ.get("VLLM_ATTN_AUTOTUNE_CACHE_DIR")
    if not d:
        return None
    h = hashlib.sha1(repr(cache_key).encode()).hexdigest()[:20]
    return os.path.join(d, f"attn_tune_{h}.json")


def _load_disk(path):
    """Rehydrate a cached result; by_bucket keys are ints but JSON stringifies them."""
    try:
        with open(path) as f:
            p = json.load(f)
        return {
            "buckets": [int(m) for m in p["buckets"]],
            "by_bucket": {int(m): k for m, k in p["by_bucket"].items()},
            "max_segments": int(p["max_segments"]),
            "prefill": p.get("prefill"),
        }
    except Exception as e:  # a stale/corrupt cache must never break startup
        logger.warning("attn autotune cache unreadable (%r); re-profiling.", e)
        return None


def _save_disk(path, result):
    if not path:
        return
    try:
        ...
        with open(tmp, "w") as f:
            json.dump(result, f)
        os.replace(tmp, path)  # atomic: concurrent TP ranks both write this
        logger.info_once("attn autotune cached to %s", path, scope="global")
    except Exception as e:
        logger.debug("attn autotune cache save failed: %r", e)
...
def profile_attn_tuning(*, num_kv_heads, num_q_heads, head_size, dtype,
    ...
    # ---- vhip PATCH: kill-switch + persistent disk cache (see header) ----
    # Returning None makes the caller fall back to built-in defaults via its existing
    # `if not result or not result.get("by_bucket")` guard — a supported path, not an error.
    if _ATTN_AUTOTUNE_OFF:
        logger.info_once("attn autotune SKIPPED (VLLM_ATTN_AUTOTUNE=0) — using built-in defaults",
                         scope="global")
        return None
    _disk = _disk_cache_path(cache_key)
    if _disk and os.path.exists(_disk):
        loaded = _load_disk(_disk)
        if loaded is not None:
            logger.info_once("attn autotune loaded from cache %s (no profiling)", _disk,
                             scope="global")
            _PROFILE_CACHE[cache_key] = loaded
            return loaded
    ...
    _save_disk(_disk, result)   # vhip PATCH: persist for the next boot
