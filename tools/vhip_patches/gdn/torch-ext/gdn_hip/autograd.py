...
The `ops.*` kernels are FORWARD-ONLY: they run the gated-delta-rule prefill, the
...
Opt-in — the inference path is untouched:
  ...
  * `enable()` registers an autograd formula on the raw ops themselves (via
    ...
    (ops.gdn_prefill(...)) becomes differentiable process-wide. Off by default.
"""
...
# Route op access through the build-unique namespace the `kernels` library generates, so this
# kernel can coexist with other builds/versions in one process (no global torch.ops.gdn_hip).
...
def _native_bwd_ok(name: str) -> bool:
    ...
    return os.environ.get("GDN_HIP_NATIVE_BWD", "1") != "0" and hasattr(ops, name)
...
class _GDNPrefillFn(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, a, b, A_log, dt_bias, scale, use_l2norm, wmma):
        ...
        op = ops.gdn_prefill_wmma if wmma else ops.gdn_prefill
...
class _CausalConv1dFwdFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        if _native_bwd_ok("causal_conv1d_bwd"):
            ...
            dx, dw, db = ops.causal_conv1d_bwd(
...
class _RMSNormGatedFn(torch.autograd.Function):
    ...
    def forward(ctx, x, z, weight, eps):
        out = ops.rmsnorm_gated(x.contiguous(), z.contiguous(), weight, float(eps))
    ...
    def backward(ctx, grad_out):
        ...
        if _native_bwd_ok("rmsnorm_gated_bwd"):
            dx, dz, dw = ops.rmsnorm_gated_bwd(
...
def enable() -> None:
    ...
    _try_register(add_op_namespace_prefix("gdn_prefill"), _prefill_backward, _prefill_setup)
    _try_register(add_op_namespace_prefix("gdn_prefill_wmma"), _prefill_backward, _prefill_setup)
    ...
    _try_register(add_op_namespace_prefix("causal_conv1d_fwd"), _conv_backward, _conv_setup)
    ...
    _try_register(add_op_namespace_prefix("rmsnorm_gated"), _norm_backward, _norm_setup)
