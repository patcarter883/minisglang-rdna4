...
# --------------------------------------------------------------------------- #
# W4A16 decode path (fp16 activations, NO activation quantisation)
# --------------------------------------------------------------------------- #
# The grouped kernels above quantise x to fp8 per token. At DECODE that buys nothing
# (measured: forcing the fp8 grouped MoE on at bs=1 gave 83.4 vs 83.4 tok/s for stock
# Triton) and it costs accuracy -- the fp8-activation degradation minisgl documents.
# `mmq_regdirect_w4a16_moe_gemv` consumes the SAME standard (E, N, K/8) int32 weights
# these hooks already materialise, so W4A16 needs no repack and no extra memory; only
# the WMMA (prefill) variant would need the lane-order `w_rep_wide` layout, which would
# double the ~8 GB/card of expert weights and is therefore NOT wired -- above the GEMV
# window we stay on the fp8-act WMMA grouped kernel (still ours, still not Triton).
_W4A16_DECISION_LOGGED = False


def _moe_w4a16_mode() -> str:
    """VLLM_ROCM_W4A16_MOE: 'off' (default) | 'auto' | 'on'.

    DEFAULT AUTO: this is now both the fastest and the most accurate MoE decode path we have.
    MEASURED on Qwen3.6-35B-A3B TP=2 bs=1 (server-side decode tok/s):

        stock Triton fused_moe (what ran before)      83.0
        W4A16, K-on-lanes gemv, unfused epilogues     71.2   (-14%)
        + BYLANE tiling for gemm2                     82.4   (parity)
        + fused SiLU gemm1 and scatter gemm2          90.7   (+9.3%)

    Two independent fixes were needed and either alone looks like a failure. (1) gemm2 (K=256,
    N=2048) starved the K-on-lanes tiling: it gives each lane 32 k-values, so a 32-lane wave only
    fills at K>=1024 and 24/32 lanes idled. BYLANE gives each LANE a whole column instead
    (74.1 -> 10.3 us/MB). (2) With that fixed the gemms were 19% cheaper than Triton yet e2e was
    still flat, because the unfused form paid a (P, 2*inter) intermediate round-trip plus a separate
    gather_reduce -- almost exactly the gemm saving. Fusing both epilogues released it.

    Accuracy is a large win on top: rel error vs an fp16 dequant reference is 0.0006 here vs 0.045
    for the fp8-activation path (tools/vhip_patches/test_w4a16_moe.py), i.e. ~70x.

    The kernel takes `at::Half` pointers, so bf16 activations are a genuine gap, not
    something to cast around -- a `.to(float16)` per layer per step is exactly the kind
    of hidden hot-path copy this wiring forbids. 'auto' therefore engages only on an
    fp16 serve and leaves the bf16 models (GLM, Laguna) on the fp8-act path; 'on'
    engages regardless and RAISES on bf16, which is what you want when you believe a
    serve is fp16 and need proof rather than a quiet no-op. Either way the decision is
    printed once at the first expert batch, so this can never go inert unnoticed."""
    v = os.environ.get("VLLM_ROCM_W4A16_MOE", "auto").strip().lower()
    if v in ("1", "on", "true"):
        return "on"
    if v in ("0", "off", "false"):
        return "off"
    if v != "auto":
        raise RuntimeError(f"VLLM_ROCM_W4A16_MOE={v!r} invalid; use auto|on|off")
    return "auto"


def _w4a16_engages(M: int, act_dtype) -> bool:
    """Whether the W4A16 GEMV path takes this batch. Called from BOTH the engage gate
    and the composition, so those two can never disagree about who owns a batch."""
    global _W4A16_DECISION_LOGGED
    mode = _moe_w4a16_mode()
    ok = mode != "off" and M <= _moe_w4a16_max_m() and (
        mode == "on" or act_dtype == torch.float16)
    if not _W4A16_DECISION_LOGGED and M <= _moe_w4a16_max_m():
        _W4A16_DECISION_LOGGED = True
        print(f"[w4a8_fp8_wmma] W4A16 MoE decode path: "
              f"{'ENGAGED' if ok else 'not engaged'} (mode={mode} act={act_dtype} "
              f"M={M} max_m={_moe_w4a16_max_m()})", flush=True)
    return ok


def _moe_w4a16_max_m() -> int:
    """Row count above which W4A16 hands off to the fp8-act WMMA path. The GEMV holds
    its M rows of x in LDS/registers (MMAX<=64) and compacts to real routed rows, so it
    owns decode and small-batch; prefill wants the tiled WMMA."""
    return int(os.environ.get("VLLM_ROCM_W4A16_MOE_MAX_M", "96"))


def _moe_w4a16_block_m() -> int:
    """moe_align block size for the W4A16 GEMV. Kernel constraint: %8==0 and <=64.
    8 matches the fp8 GEMV default -- it caps real rows/block so the acc[COLS][MMAX]
    register footprint stays small; extra per-expert rows spill into more blocks."""
    return int(os.environ.get("VLLM_ROCM_W4A16_MOE_BLOCK_M", "8"))
...
def _moe_should_engage(M, w13_op, w2_op, w13_s, topk_ids, layer=None,
                       act_dtype=None) -> bool:
    ...
    W4A16 OVERRIDES ALL OF THAT for M <= _moe_w4a16_max_m(). The crossover cache only
    ever described the fp8-ACTIVATION kernel, whose measured decode window is empty --
    which is exactly why every bs=1 batch fell through to stock Triton fused_moe. The
    W4A16 GEMV is a different kernel with a different crossover, so consulting that
    cache for it would be answering the wrong question. It engages unconditionally in
    its window; _run_grouped_moe re-checks the dtype/shape preconditions and RAISES if
    they are unmet rather than silently deferring."""
    if _w4a16_engages(M, act_dtype):
        return True
    force = _force_mode()
    if force == "on":
        return True
...
# --------------------------------------------------------------------------- #
def _w4a16_require(cond: bool, msg: str) -> None:
    if not cond:
        raise RuntimeError(
            f"W4A16 MoE: {msg}. Refusing rather than silently falling back -- three separate "
            "wirings in this stack shipped INERT because they degraded quietly. Set "
            "VLLM_ROCM_W4A16_MOE=0 to run the fp8-activation grouped kernel instead.")


def _run_grouped_moe_w4a16(x, w1, w2, w1_s, w2_s, w1_zp, w2_zp,
                           topk_weights, topk_ids, activation,
                           global_num_experts, expert_map,
                           apply_router_weight_on_input, out_dtype=None,
                           weight_is_e2m1=False):
    """True W4A16 gated MoE: int4 weights, **fp16 activations, no activation quant**.

    Identical composition to _run_grouped_moe, but every GEMM is the register-direct
    W4A16 GEMV, which reads the SAME standard ``(E, N, K//8)`` int32 weights and applies
    only the weight scale in its epilogue (no per-token activation scale round-trip):

        moe_align -> gemv(w13) -> gated activation -> gemv(w2) -> topk gather-reduce

    gemm2 uses the NON-scatter GEMV plus ``mmq_fp8_moe_gather_reduce`` (that reduction
    Both epilogues are FUSED: gemm1 writes silu(gate)*up directly (no (P, 2*inter)
    intermediate, no separate activation launch) and gemm2 folds the topk weight and the
    scatter-reduce into its epilogue (no gather_reduce). Unfused, those two round-trips
    cost about as much as the gemms save.

    NO CASTS: x must already be fp16 and the scales must already be fp16 (the hooks
    convert scales ONCE at load). NO FALLBACK: anything unsupported raises."""
    ...
    # The kernel is fp16-typed (`at::Half` data_ptr). A `.to(float16)` here would be a
    # real cast+copy on the hot path under a bf16 serve, so refuse instead: bf16 + W4A16
    # is a genuine kernel gap, not something to paper over.
    _w4a16_require(x.dtype == torch.float16,
                   f"activations are {x.dtype}, kernel requires float16 (serve --dtype float16)")
    _w4a16_require(w1_s.dtype == torch.float16 and w2_s.dtype == torch.float16,
                   f"scales are {w1_s.dtype}/{w2_s.dtype}, kernel requires float16")
    inter2 = w1.size(1)        # 2 * inter (gate|up)
    inter = w2.size(2) * 8     # w2 is (E, K, inter//8)
    group1 = K // w1_s.size(2)
    group2 = inter // w2_s.size(2)
    _w4a16_require(K % 32 == 0 and inter % 32 == 0,
                   f"K={K} and inter={inter} must both be multiples of 32")
    _w4a16_require(group1 % 32 == 0 and group2 % 32 == 0,
                   f"group sizes {group1}/{group2} must be multiples of 32")
    block_m = _moe_w4a16_block_m()
    _w4a16_require(block_m % 8 == 0 and 0 < block_m <= 64,
                   f"block_m={block_m} must be a multiple of 8 and <= 64")
    ...
    # gemm1 (w13): gathers x by sorted_token_ids//top_k. FUSED SiLU writes the post-activation
    # (P, inter) straight from the epilogue -- no separate activation launch and, more importantly,
    # no (P, 2*inter) intermediate write+read. MEASURED: the unfused form handed its entire gemm
    # win back to that round-trip plus the gather-reduce, landing at parity with stock Triton.
    # Non-SILU gated activations (GELU etc.) have no fused kernel, so they keep the two-step form --
    # a genuine capability gap, not a silent degrade, and still entirely on our kernels.
    from vllm.model_executor.layers.fused_moe.activation import MoEActivation
    if activation.is_gated and activation == MoEActivation.SILU:
        buf2 = w4a8_fp8_wmma.mmq_regdirect_w4a16_moe_gemv_silu(
            x_in, w1, w1_s, sorted_ids, expert_ids, ntp,
            top_k, block_m, w_zeros=w1_zp, weight_is_e2m1=weight_is_e2m1)   # (P, inter)
    else:
        out1 = w4a8_fp8_wmma.mmq_regdirect_w4a16_moe_gemv(
            x_in, w1, w1_s, sorted_ids, expert_ids, ntp,
            inter2, top_k, block_m, w_zeros=w1_zp, weight_is_e2m1=weight_is_e2m1)
    ...
    # gemm2 (w2) with the FUSED topk-weight + scatter epilogue, replacing the separate
    # mmq_fp8_moe_gather_reduce. The output must be pre-zeroed, and that zeroing happens INSIDE the
    # captured graph, which is exactly what makes the atomicAdd graph-safe (the fp8 use_gemv path
    # has shipped this under capture all along; an earlier note here claiming the scatter was not
    # capturable was wrong). NOTE top_k is the REAL top_k, not 1: SCATTER takes the A-row by padded
    # index (identity gather) and uses top_k only to map sorted offset -> token.
    output = torch.zeros((M, K), dtype=torch.float32, device=dev)
    w4a8_fp8_wmma.mmq_regdirect_w4a16_moe_gemv_scatter(
        buf2.contiguous(), w2, w2_s, sorted_ids, expert_ids, ntp, tw_flat, output,
        K, top_k, block_m, w_zeros=w2_zp, weight_is_e2m1=weight_is_e2m1)
...
def _run_grouped_moe(x, w1, w2, w1_s, w2_s, w1_zp, w2_zp,
    ...
    # W4A16 owns the decode window: same weights, fp16 activations, no act-quant.
    # Biased experts (gpt-oss mxfp4) stay on the fp8 path -- the GEMV epilogue has no
    # per-expert bias, and that path already materialises the (P, N) buffer to add it.
    if (w1_bias is None and w2_bias is None
            and _w4a16_engages(M, x.dtype)):
        return _run_grouped_moe_w4a16(
            x, w1, w2, w1_s, w2_s, w1_zp, w2_zp, topk_weights, topk_ids, activation,
            global_num_experts, expert_map, apply_router_weight_on_input,
            out_dtype=out_dtype, weight_is_e2m1=weight_is_e2m1)
...
def register_moe_mxfp4(verbose: bool = True) -> bool:
    ...
    try:
        ...
        from vllm.model_executor.layers.fused_moe.activation import MoEActivation
        from w4a8_vllm.mxfp4_convert import convert_mxfp4_moe
