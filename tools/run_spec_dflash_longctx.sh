#!/usr/bin/env bash
# Launcher for spec_dflash_longctx_lean.sh inside minisgl-rdna4:lean, TP=2. Mounts THIS WORKTREE.
#   gpu-lease -n 2 -- bash <worktree>/tools/run_spec_dflash_longctx.sh
set -uo pipefail
WORKTREE="$(cd "$(dirname "$0")/.." && pwd)"
HIP="${HIP_VISIBLE_DEVICES:-${LEASE_HIP_DEVICES:-0}}"
ROCR="${ROCR_VISIBLE_DEVICES:-${LEASE_ROCR_DEVICES:-0}}"
echo "[run_dflash_longctx] WORKTREE=$WORKTREE HIP=$HIP ROCR=$ROCR"
...
docker run --rm \
  ...
  -e LONGTOK="${LONGTOK:-4000}" \
  -v "$WORKTREE":/engine -v "$TRITON_COPY":/root/.triton \
  -v /home/pat/.cache/huggingface:/root/.cache/huggingface -e HF_HUB_OFFLINE=1 \
  -e PYTHONPATH=/opt/kernels:/engine/python:/engine \
  --entrypoint bash minisgl-rdna4:lean -lc 'bash /engine/tools/spec_dflash_longctx_lean.sh'
echo "[run_dflash_longctx] exited rc=$?"
