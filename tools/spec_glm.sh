#!/usr/bin/env bash
# MLA spec-decode validation on GLM-4.7-Flash (glm4_moe_lite, AWQ, MLA) at TP=2. Runs INSIDE
# vllm22-w4a8:combined under a 2-card lease. Verifies:
#   1. coherence with spec on (absorbed multi-query mla_verify kernel);
#   2. losslessness: SPEC (multi-token accept) == FORCE_N0 (1 token/step, same verify kernel).
# Eager (--graph 0), MLA backend auto-forced (no --attention-backend), page_size stays 16.
set -uo pipefail
...
MODEL="${MODEL:-QuantTrio/GLM-4.7-Flash-AWQ}"; PORT=21939; LOG=/engine/tools/spec_glm.server.log
...
echo "===== SPEC (MLA absorbed multi-query verify) ====="
boot "MINISGL_SPEC_DEBUG=1"; probe /engine/tools/glm.spec.json
echo "[spec] acceptance + backend:"; grep -E "\[spec\]" "$LOG" | tail -3
grep -iE "page.?size|overrid|backend" "$LOG" | grep -iE "page|backend|spec|mla" | head -4; stop

echo "===== FORCE_N0 (1 token/step, same mla_verify kernel) ====="
boot "MINISGL_SPEC_FORCE_N0=1"; probe /engine/tools/glm.n0.json; stop

echo "===== DIFF spec vs n0 (must match -> MLA accept/rollback lossless) ====="
python - /engine/tools/glm.spec.json /engine/tools/glm.n0.json <<'PY'
import json,sys
...
print("\nMLA ACCEPT/ROLLBACK LOSSLESS:", "PASS" if ok else "FAIL")
PY
echo "[done]"
