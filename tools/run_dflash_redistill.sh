...
NUM_SPEC="${NUM_SPEC:-4}"; EPOCHS="${EPOCHS:-14}"; BATCH="${BATCH:-512}"; LR="${LR:-5e-5}"; SAVE_EPOCHS="${SAVE_EPOCHS:-}"
...
docker run --rm --name "$CNAME" \
  ...
  -e INIT="$INIT" -e OUT="$OUT" -e NUM_SPEC="$NUM_SPEC" -e EPOCHS="$EPOCHS" -e BATCH="$BATCH" -e LR="$LR" -e NANDIAG="${NANDIAG:-0}" -e SAVE_EPOCHS="$SAVE_EPOCHS" \
  ...
  --entrypoint bash "${MINISGL_IMAGE:-minisgl-rdna4:lean}" -lc '
    ...
    python /trainrepo/zaya/dflash/train_cca_drafter.py \
      ...
      --num-spec "$NUM_SPEC" --epochs "$EPOCHS" --batch "$BATCH" --lr "$LR" ${SAVE_EPOCHS:+--save-epochs "$SAVE_EPOCHS"}
  '
